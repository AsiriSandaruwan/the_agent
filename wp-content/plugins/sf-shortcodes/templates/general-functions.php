...
function service_finder_get_rquest_data( $obj = array(), $key = '', $default = '' )
{
	if( !$obj )
	{
		return false;
...
function service_finder_get_statistics_data( $boxnum = '' )
{
	global $service_finder_options;
	
	$type = service_finder_get_rquest_data($service_finder_options,'shortcode-statistics-box'.$boxnum.'-type');
	$providerreplacestring = service_finder_get_rquest_data($service_finder_options,'provider-replace-string',esc_html__('Providers', 'service-finder'));
	$customerreplacestring = service_finder_get_rquest_data($service_finder_options,'customer-replace-string',esc_html__('Customers', 'service-finder'));
	$text = service_finder_get_rquest_data($service_finder_options,'shortcode-statistics-box'.$boxnum.'-title');
	$number = service_finder_get_rquest_data($service_finder_options,'shortcode-statistics-box'.$boxnum.'-number');
	$colorcode = service_finder_get_rquest_data($service_finder_options,'shortcode-statistics-box'.$boxnum.'-color');
	...
	switch($type){
		...
		case 'jobs': 
			...
			$text = esc_html__('Jobs', 'service-finder');
			break;
		case 'categories': 
			if($number == 0 || $number == ''){
			...
			$text = esc_html__('Categories', 'service-finder');
			break;
		default:

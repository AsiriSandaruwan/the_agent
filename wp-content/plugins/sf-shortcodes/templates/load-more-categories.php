...
/*theme style 4*/
add_action('wp_ajax_load_more_v4', 'service_finder_load_more_v4');
add_action('wp_ajax_nopriv_load_more_v4', 'service_finder_load_more_v4');
function service_finder_load_more_v4(){
global $service_finder_options;
		...
		$getoffset = (!empty($_POST['offset'])) ? $_POST['offset'] : '';
                ...
                if(!empty($categories)){
					...
					if($totalcat > $newoffset){
					if(!empty($catdetails)){
					echo '<div class="show_more_main" id="show_more_main'.esc_attr($newoffset).'"> <span id="'.esc_attr($catdetails->term_id).'" data-catarr="yes" data-offset="'.esc_attr($newoffset).'" class="show_more2 btn btn-primary" title="Load more categories"><i class="fa fa-refresh"></i> '.esc_html__('Show more','service-finder').'</span> <span class="loding default-hidden"><span class="loding_txt btn btn-default"><i class="fa fa-refresh fa-spin"></i></span></span> </div>';
					}
					}
					...
					if(!empty($categories)){
						...
						foreach($categories as $category){
							...
							if($totalcat > $newoffset){
							echo '<div class="show_more_main" id="show_more_main'.esc_attr($newoffset).'"> <span id="'.esc_attr($category->term_id).'" data-offset="'.esc_attr($newoffset).'" class="show_more2 btn btn-primary" title="Load more posts"><i class="fa fa-refresh"></i> '.esc_html__('Show more','service-finder') .'</span> <span class="loding default-hidden"><span class="loding_txt btn btn-default"><i class="fa fa-refresh fa-spin"></i></span></span> </div>';
							} 
